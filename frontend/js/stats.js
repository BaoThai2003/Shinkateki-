...
function renderWeaknessGrid(chars) {
  ...
  if (!chars.length) {
    grid.innerHTML = '<p style="color:var(--fog);font-size:.9rem;">No data yet.</p>';
    return;
  }
...
function renderTimeOfDay(insights) {
  ...
  if (!insights.length) {
    grid.innerHTML = '<p style="color:var(--fog);font-size:.9rem;">Study at different times to unlock this analysis.</p>';
    return;
  }
    ...
    return `
      <div class="tod-card ${isBest ? 'tod-best' : ''}">
        ...
        <div style="font-size:.7rem;color:var(--fog);margin-top:.4rem;font-family:var(--font-mono)">
          ${item.totalAttempts} attempts · ${Math.round(item.avgResponseMs / 100) / 10}s avg
        </div>
        ${isBest ? '<div style="font-size:.7rem;color:#9de0b8;margin-top:.25rem">★ best time</div>' : ''}
      </div>
    `;
  }).join('');
}
...
function renderVelocity(v) {
  ...
  if (!v || v.status === 'insufficient_data') {
    card.innerHTML = '<p style="color:var(--fog);font-size:.9rem;">Complete at least 10 attempts to unlock velocity tracking.</p>';
    return;
  }
